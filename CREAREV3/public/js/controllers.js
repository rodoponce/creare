...
function ValidarCampos() {
    ...
    var fechanac= $scope.sampleDate;

    var anio= parseInt(fechanac.slice(6,11));
    var mes= fechanac.slice(3,5);
    var dia=  fechanac.slice(0,2);

     
    if((2018-anio)>=8 && (2018-anio)<=80){
        $scope.errorFecha=false;
        var date = new Date(anio,mes,'0');

        if((dia-0)>(date.getDate()-0)){
              $scope.errorFecha=true;
              Alertnoty("Fecha no válida");
        }else{
           $scope.errorFecha=false;
        }
...
if(  Nacionalidad!="Mexico" && Tipo!="Estudiante" )
{
   ...
   function UniversidadCheck()
   {
       if($scope.Miuniversidad=="")
       {
           ...
           Alertnoty("Seleccione Universidad");      
        }
   }
   ...
   function ResidencialData()
   {
       ...
       if(al==1)
       {
        Alertnoty("Falta Datos Residenciales");    
       }
   }
   ...
   function CheckInformacion()
   {
    ...
    //no hace la comparacion 
        if($scope.Ic==undefined || $scope.Imm==undefined )
        {
...
$scope.Pruebas=function(){
    ...
    console.log($scope.marca, $scope.RFC);
...
function DiseñadorExtranjero()
{
    ...
    $scope.SeleccionPais= function()
    {
        ...
        var Seleccion='';
        if ($scope.MiOcupacion.nombre != undefined){
            Seleccion = $scope.MiOcupacion.nombre;
        }
